...
import jikanpy
import requests
from AstrakoBot import DEV_USERS, OWNER_ID, SUDO_USERS, dispatcher
...
def site_search(update: Update, context: CallbackContext, site: str):
    ...
    if site == "kaizoku":
        search_url = f"https://Astrako.com/?s={search_query}"
        html_text = requests.get(search_url).text
        soup = bs4.BeautifulSoup(html_text, "html.parser")
        search_result = soup.find_all("h2", {'class': "post-title"})

        if search_result:
            result = f"<b>Search results for</b> <code>{html.escape(search_query)}</code> <b>on</b> <code>Astrako</code>: \n"
            ...
            more_results = False
            result = f"<b>No result found for</b> <code>{html.escape(search_query)}</code> <b>on</b> <code>Astrako</code>"
...
*Available commands:*
 ...
 • `/kaizoku <anime>`*:* search an anime on Astrako.com
